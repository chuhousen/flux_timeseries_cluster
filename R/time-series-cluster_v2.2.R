...
n.grp <- seq(10, 30, by = 1)  # searching window for the n of clusters/branches to keep 
...
drop.ls <- NULL #c("FC", "SC")

# target variables to run uni-variate clustering
target.var.ls <- c("TA_F", "SW_IN_F", "LW_IN_F", "VPD_F",
                   "LE_F_MDS", "H_F_MDS", "NEE_VUT_REF",
                   "RECO_NT_VUT_REF", "GPP_NT_VUT_REF",
                   "RECO_DT_VUT_REF", "GPP_DT_VUT_REF")
target.lab.ls <- list(expression(TA~'('*degree~C*')'),
                      expression(SW_IN~'('*W~m^{-2}*')'),
                      expression(LW_IN~'('*W~m^{-2}*')'),
                      expression(VPD~'('*hPa*')'),
                      expression(LE~'('*W~m^{-2}*')'),
                      expression(H~'('*W~m^{-2}*')'),
                      expression(NEE~'('*mu*mole~m^{-2}~s^{-1}*')'),
                      expression(RECO_NT~'('*mu*mole~m^{-2}~s^{-1}*')'),
                      expression(GPP_NT~'('*mu*mole~m^{-2}~s^{-1}*')'),
                      expression(RECO_DT~'('*mu*mole~m^{-2}~s^{-1}*')'),
                      expression(GPP_DT~'('*mu*mole~m^{-2}~s^{-1}*')')
                      )
target.rng.ls <- list(seq(-40, 40, length.out = 5),
                      seq(0, 1500, length.out = 6),
                      seq(200, 600, length.out = 5),
                      seq(0, 60, length.out = 5),
                      seq(-300, 900, length.out = 5),
                      seq(-300, 900, length.out = 5),
                      seq(-40, 40, length.out = 5),
                      seq(-20, 80, length.out = 6),
                      seq(-30, 120, length.out = 6),
                      seq(-20, 80, length.out = 6),
                      seq(-30, 120, length.out = 6)
                      )
...
for (l1 in 1:length(target.var.ls)) {
  ...
  for(dd in 1: length(dist.ls)){
    ...
    for (j2 in 1:n.grp.opt) {
      ...
      data.pre.sub.mean <- hc@centroids[[grp.ls.sort[j2]]]
    ...
    if (length(sub.grp.ls) > 0) {
      for (i in 1:length(sub.grp.ls)) {
        data.pre.sub1 <- as.data.frame(data.pre[, which(grp.ls == sub.grp.ls[i])])
        ...
        png(
          paste0(path.out, "AMF-diurnal-seasonal-cluster-", target.var, "_subgroup_", sub.grp.ls[i], "-", target.dist, ".png"),
          width = 11,
          height = 9,
          units = "in",
          pointsize = 9,
...
write.csv(
  full.ls,
  paste0(path.out, "ALL_FLUXNET_site_short_list2.csv"),
  quote = T,
  row.names = F
)
