...
drop.ls <- c("US-GBT", "US-PFa", ## unique site
             ...
             "MX-Tes", "CA-NS8" ## forest sites with no SC nor EC height info
              ) 
...
## data availability
data.yr.ameriflux <- jsonlite::fromJSON(httr::content(
  httr::POST("https://ameriflux-data.lbl.gov/AmeriFlux/SiteSearch.svc/PublishYears/AmeriFlux"),
  as = "text"
),  flatten = TRUE)

sgi.amf <- merge.data.frame(sgi.amf, data.yr.ameriflux)
sgi.amf$DATA_START <- sapply(sgi.amf$publish_years, na.min)
sgi.amf$DATA_END <- sapply(sgi.amf$publish_years, na.max)
...
var.info.tmp <- get_latest_VI(path.out = RDir)
...
full.ls$GRP_LOCATION.LOCATION_LAT <- as.numeric(full.ls$GRP_LOCATION.LOCATION_LAT)
full.ls$GRP_LOCATION.LOCATION_LONG <- as.numeric(full.ls$GRP_LOCATION.LOCATION_LONG)
full.ls$GRP_LOCATION.LOCATION_ELEV <- as.numeric(full.ls$GRP_LOCATION.LOCATION_ELEV)
...
for (k in 1:length(target.site)) {
  ...
  SW_IN_POT <- ext_radiation_v2(
    latitude = full.ls$GRP_LOCATION.LOCATION_LAT[k],
    longitude = full.ls$GRP_LOCATION.LOCATION_LONG[k],
    ...
    if(length(sc.loc) == 0){
      ...
      if (length(co2.loc) > 0 &
        ...
        if (sum.notna(site.zm.list$htower) > 0 &
          ...
          } else if (!is.na(full.ls$GRP_LOCATION.LOCATION_ELEV[k])) {
            pa.data <-
              101.3 * exp(-full.ls$GRP_LOCATION.LOCATION_ELEV[k] / 8200)
