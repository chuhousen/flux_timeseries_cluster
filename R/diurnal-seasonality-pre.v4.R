...
source("get_full_list.R")
source("grab_version.R")
source("basename_pharse.R")
source("rename_aggregate.R")
source("simple_aggregate.R")
source("filter_physical_range.R")
source("plot_diurnal_seasonal.R")
source("get_latest_VI.R")
source("work_on_zm_list.R")

path<-"D:\\Housen\\Flux\\Data-exploring\\00_Synthesis_AmeriFlux_Time_Series_Cluster\\"
base.in<-"D:\\AmeriFlux-Data\\00_olaf_data_ameriflux\\BASE-BADM\\"
path.out.root<-paste(path,"diurnal-seasonality\\",sep="")
...
#sgi.amf$SITE_ID[which(sgi.amf$SITE_ID=="US-SdH")]<-paste("US-Sdh")
...
ver<-"20201117"     # for storing outputs

if(!dir.exists(paste(path.out.root,ver,sep=""))) dir.create(paste(path.out.root,ver,sep=""))
...
hr.res<-"to_hr"         # org: maintain original time resolution (HH/HR) in generating diel stat
...
VPD.need.correct<-c("US-CF1","US-RC4","US-RC3","US-RC2",
                    "US-RC5","US-CF2","US-CF3","US-CF4","US-RC1")

allow.nongap.within.wd<-0.66  ## min percentage of non-missing in filling diurnal-seasonal time series, within each window
allow.nongap.across.wd<-0.80  ## min percentage of non-missing in filling diurnal-seasonal time series, across window

### FP-Standard variable list
FP.ls<-read.csv(paste(path,"flux-timeseries-cluster\\AMF-FP-Variable_20181011.csv",sep=""),header=F,na=NA,stringsAsFactors=F)
colnames(FP.ls)<-c("name","description","units")

### Predefined physical range
#limit.ls<-read.csv(paste(path,"flux-timeseries-cluster\\Physical_Range_20170710.csv",sep=""),header=T,na=NA,stringsAsFactors=F)
limit.ls <- jsonlite::fromJSON(content(
...
target.site<-get_full_list(base.in)   ## could be full list (intial pull out), pull from get_full_list function
...
target.res[which(target.site=="BR-Sa1"|target.site=="US-MMS"|target.site=="US-Ha1"|
                   target.site=="US-Cop"|target.site=="US-Ne1"|target.site=="US-Ne2"|
                   target.site=="US-Ne3"|target.site=="US-PFa"|target.site=="US-Cwt")]<-"HR"

### Search within BASE-BADM files, looking for latest version 
src.list<-list.files(base.in)
src.list<-src.list[which(substr(src.list,start=nchar(src.list)-3,stop=nchar(src.list))==".zip")]
src.list<-substr(src.list,start=1,stop=nchar(src.list)-4)

src.list<-src.list[-which(substr(src.list,
                                 start=nchar(src.list)-3,
                                 stop=nchar(src.list))=="None")]
## Prepare a full site list
full.ls<-sgi.amf[target.site,
                 c("SITE_ID","IGBP","GRP_LOCATION.LOCATION_LAT","GRP_LOCATION.LOCATION_LONG",
                   "GRP_LOCATION.LOCATION_ELEV","GRP_CLIM_AVG.MAT","GRP_CLIM_AVG.MAP","GRP_CLIM_AVG.CLIMATE_KOEPPEN")]
full.ls$data_year_length<-NA
full.ls.col<-ncol(full.ls)
full.ls<-data.frame(full.ls,matrix(0,nrow=nrow(full.ls),ncol=length(sel.var)*2))
colnames(full.ls)[c((full.ls.col+1):ncol(full.ls))]<-paste(rep(sel.var,each=2),c("","_F"),sep="")

if(sink.log.to.file){
  sink(file=paste(path.out,substr(Sys.time(),start=1,stop=10),"-diurnal-seasoal-pre-log.txt",sep=""))
}
...
for(k in 1:length(target.site)){
  ...
  ## control var, used in preparing the diel-seasonal outputs  
  ...
  base.list.tmp<-src.list[which(substr(src.list,start=5,stop=10)==target.site[k])]
  
  target.base.ver<-latest.two(grab.version(base.list.tmp)[[1]],grab.version(base.list.tmp)[[2]])[1] 
  base.list<-paste("AMF_",target.site[k],"_BASE-BADM_",target.base.ver,sep="")
  
  ## speify the BASE files under zip to be grabbed 
  case.ls<-NULL
  for(j in 1:length(target.base.ver)){
    file.grab<-unzip(paste(base.in,base.list[j],".zip",sep=""),list=TRUE)[,"Name"]
    case.ls<-c(case.ls,file.grab[which(substr(file.grab,start=12,stop=15)=="BASE"&
                                         substr(file.grab,start=17,stop=18)==target.res[k])])
  }
  case.ls<-substr(case.ls,start=1,stop=nchar(case.ls)-4)
  
  ## newest BASE
  case.name<-base.list[1]
  data.in<-read.table(unz(paste(base.in,case.name,".zip",sep=""),paste(case.ls[1],".csv",sep="")),
                      na=c("-9999"),header=T,sep=",",skip=2,stringsAsFactors=F)  
  
  ## used for outputs
  case.name.short<-paste(target.site[k],"_",target.res[k],"_",target.base.ver,sep="")
  ...
  all.empty<-which(apply(data.in[,c(3:ncol(data.in))],2,sum.notna)==0)+2
  ...
  if(nrow(data.in)<(365*d.hr.org)){
    ...
    #################################################################################################################
    ## This part pharses variable names/qulifiers
    
    ## decode the variable name, info of aggregation level, gap-filling,...
    
    basename_decode<-basename_pharse(var.name=var.name,
                                     FP.ls=FP.ls,
                                     echo=T)
    
    write.csv(basename_decode,
              paste(path.out,case.name.short,"_basename_decode.csv",sep=""),quote=F,row.names=F)
    
    #################################################################################################################
    ## This part filter data based on the physical range 
    
    data.work<-filter_physical_range(data.in=data.in,
    ...
    data.work.holder<-rename_aggregate(basename_decode=basename_decode,
                                       data.in=data.work)
    ...
    data.work.holder<-simple_aggregate(basename_decode.work=data.work.holder[[2]],
    ...
    data.work<-data.frame(TIMESTAMP=NA,DATE_ID=NA,HOUR_ID=NA,
                          data.work)
    
    if(length(which(colnames(data.work)=="TIMESTAMP_START"))==1){
      data.work$TIMESTAMP<-strptime(data.work$TIMESTAMP_START,format="%Y%m%d%H%M",tz="UTC")
      data.work$TIMESTAMP<-strptime(data.work$TIMESTAMP+0.5*hr*60,format="%Y-%m-%d %H:%M:%S",tz="UTC")
    }else if(length(which(colnames(data.work)=="TIMESTAMP_END"))==1){
      data.work$TIMESTAMP<-strptime(data.work$TIMESTAMP_END,format="%Y%m%d%H%M",tz="UTC")
      data.work$TIMESTAMP<-strptime(data.work$TIMESTAMP-0.5*hr*60,format="%Y-%m-%d %H:%M:%S",tz="UTC")
    ...
    #################################################################################################################
    ## This part create holder for working diel-seasonal outputs
    
    ## Create time id for diel-seasonl aggregation, apply func
    data.work$DATE_ID<-floor(data.work$TIMESTAMP$yday/l.wd)+1
    data.work$DATE_ID[which(data.work$DATE_ID==max(data.work$DATE_ID,na.rm=T))]<-n.wd
    # wrap all hanging days into last window
    
    if(hr.res=="org"){
      data.work$HOUR_ID<-data.work$TIMESTAMP$hour+data.work$TIMESTAMP$min/60
    }else if(hr.res=="to_hr"){
      data.work$HOUR_ID<-data.work$TIMESTAMP$hour+30/60
    }
    
    basename_decode.work<-basename_pharse(var.name=colnames(data.work),
                                          FP.ls=FP.ls,
                                          echo=F)
    ...
    rh.loc<-which(basename_decode.work$basename=="RH"&
                    !basename_decode.work$is_gapfill)
    ta.loc<-c(which(basename_decode.work$basename=="TA"&
                    !basename_decode.work$is_gapfill),
              which(basename_decode.work$basename=="T_SONIC"&
                      !basename_decode.work$is_gapfill))
    ...
    if(length(vpd.loc)>0){
      ...
      data.work$VPD <-(0.612*exp(17.27*data.work[,ta.loc[1]]/(data.work[,ta.loc[1]]+237.3)))*
...
#       }
    ...
    for(j1 in 1:length(sel.var)){
      ...
      if(length(sel.var.loc)==1){ ## if single/unique culumn 
        
        ## get data availability
        full.ls[k,(full.ls.col+2*j1-1)]<-sum.notna(data.work[,sel.var.loc])/nrow(data.work)
        
        for(i in 1:n.wd){
          #temp.out1[,i]<-c(tapply(data.work[data.work$DATE_ID==i,sel.var.loc],
          #                        data.work$HOUR_ID[data.work$DATE_ID==i],
          #                        low.bd2))
          temp.out2[,i]<-c(tapply(data.work[data.work$DATE_ID==i,sel.var.loc],
                                  data.work$HOUR_ID[data.work$DATE_ID==i],
                                  low.bd))
          temp.out3[,i]<-c(tapply(data.work[data.work$DATE_ID==i,sel.var.loc],
                                  data.work$HOUR_ID[data.work$DATE_ID==i],
                                  na.median))
          temp.out4[,i]<-c(tapply(data.work[data.work$DATE_ID==i,sel.var.loc],
                                  data.work$HOUR_ID[data.work$DATE_ID==i],
                                  upp.bd))
          ...
          #                        upp.bd2))
               ...
               sum(basename_decode.work$aggregate_method[sel.var.loc]=="naive")==length(sel.var.loc)){  ## if all layer-aggregted 
        
        ## locate the top level
        sel.var.loc.top<-sel.var.loc[which(basename_decode.work$layer[sel.var.loc]=="1")]
        
        ## get data availability
        full.ls[k,(full.ls.col+2*j1-1)]<-sum.notna(data.work[,sel.var.loc.top])/nrow(data.work)
        
        if(length(sel.var.loc.top)==1){  ## if single uniue top-layer 
          for(i in 1:n.wd){
            #temp.out1[,i]<-c(tapply(data.work[data.work$DATE_ID==i,sel.var.loc.top],
            #                        data.work$HOUR_ID[data.work$DATE_ID==i],
            #                        low.bd2))
            temp.out2[,i]<-c(tapply(data.work[data.work$DATE_ID==i,sel.var.loc.top],
                                    data.work$HOUR_ID[data.work$DATE_ID==i],
                                    low.bd))
            temp.out3[,i]<-c(tapply(data.work[data.work$DATE_ID==i,sel.var.loc.top],
                                    data.work$HOUR_ID[data.work$DATE_ID==i],
                                    na.median))
            temp.out4[,i]<-c(tapply(data.work[data.work$DATE_ID==i,sel.var.loc.top],
                                    data.work$HOUR_ID[data.work$DATE_ID==i],
                                    upp.bd))
          ...
          print(paste("[Warning] can't locate top-layer",
                      paste(basename_decode.work$variable_names[sel.var.loc],collapse=" ")))
        }
        
      }else if(length(sel.var.loc)>0){  # catch up cases that variables exist, but not unique or aggregated to laters
      ...
      for(i3 in 1:2){ 
        ...
        for(i in 1:n.wd){
          if(sum(!is.na(temp.out3[,i]))!=d.hr&
             sum(!is.na(temp.out3[,i]))>allow.nongap.within.wd*d.hr/i3){
      ...
      full.ls[k,(full.ls.col+2*j1)]<-sum.notna(c(temp.out3))/length(temp.out3)
    ...
    plot_diurnal_seasonal(data.median=data.out3,
                          data.upp1=data.out2,
                          #data.upp2=data.out1,
                          data.low1=data.out4,
                          #data.low2=data.out5,
                          ...
                          l.wd=l.wd,
                          skip.var.loc=c(1:2),
                          n.data.yr=floor(nrow(data.work)/d.hr.org/365))
    
    #write.csv(data.out1,file=paste(path.out,case.name.short,"_LOWER2.csv",sep=""),quote=F,na="-9999",row.names=F)
    write.csv(data.out2,file=paste(path.out,case.name.short,"_LOWER1.csv",sep=""),quote=F,na="-9999",row.names=F)
    write.csv(data.out3,file=paste(path.out,case.name.short,"_MEDIAN.csv",sep=""),quote=F,na="-9999",row.names=F)
    write.csv(data.out4,file=paste(path.out,case.name.short,"_UPPER1.csv",sep=""),quote=F,na="-9999",row.names=F)
...
write.csv(full.ls,paste(path.out,"ALL_BASE_site_list.csv",sep=""),quote=F,na="-9999",row.names=F)
...
select.ls<-full.ls[(full.ls$LE_F==1&full.ls$H_F==1)|(full.ls$NEE_F==1&full.ls$H_F==1)|(full.ls$LE_F==1&full.ls$NEE_F==1),]
write.csv(select.ls,paste(path.out,"ALL_BASE_site_short_list.csv",sep=""),quote=F,na="-9999",row.names=F)
