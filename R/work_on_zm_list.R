work_on_zm_list<-function(work.path,    ## where a previous version of zm list exist
  ...
  zm.list <-
    read.csv(
      paste(work.path, "FLX_BADM_EC_HEIGHT-20180722.csv", sep = ""),
      header = T,
      stringsAsFactors = F
    )
  ...
  if (!target.site %in% zm.list$site) {
    ...
    if (nrow(var.info.grab) > 0) {
      ...
      basename_decode <-
        basename_pharse(var.name = var.info.grab$Variable,
                        FP.ls = FP.ls,
                        echo = F)
      
      ec.loc <-
        as.numeric(rownames(basename_decode)[which(
          basename_decode$basename == "USTAR" |
